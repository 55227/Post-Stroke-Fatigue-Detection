...
class RNN(nn.Module):
    ...
    def _get_rnn(self, arch, arch_args):
        ...
        return {
            'lstm': nn.LSTM,
            'gru': nn.GRU
        }[arch](**arch_args)# `**` used to unpack the dictionary into keyword arguments
    ...
    def forward(self, x):
        ...
        self.rnn.flatten_parameters()# used to optimize the memory usage when training by flattening the weight parameters to reduce the overhead in the backward pass
        return self.rnn(x)
